...
void PeopleSegmentation::segmentPeople(const Mat& original,Mat& dest) {
	...
	The following method to extract the dominant background color from the inverted mask performed well only for the people-segmentation task, but when we insert it, the pipeline has an uncontrolled behavior, so it works only locally  */
