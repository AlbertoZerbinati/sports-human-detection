...
Mat GreenFieldsSegmentation(const Mat &I /*, String filename*/)
{
    ...
    Mat I_gca;
    green_chromaticity_analysis(I_open, I_gca); // image with values in range 0-255, ready for view
    ...
    Mat samples = I_gca_reduced.reshape(1, I_gca_reduced.rows * I_gca_reduced.cols);

    // Number of Gaussian distributions used for the E-M algorithm.
    int N_G = 6;

    Mat log_likelihoods, labels, probs;
    Ptr<ml::EM> gmm = gmm_load_trained(samples, N_G, log_likelihoods, labels, probs);
    ...
    // Now choose T_G
    double T = 1.0 / 4.0;
    double m0 = findFirstMinimumAfterIndex(pdf, g, findFirstMaximumAfterThreshold(pdf, g, T));
    ...
    // imwrite("../results_images/" + filename + "_inverted_field" + ".jpg", result_inverted);
    // imwrite("../results_images/" + filename + "_mask" + ".jpg", mask2);
    return mask2.clone();
}
...
Mat GenericFieldSegmentation(Mat &image, int from_row, int from_column, int row_width, int column_width, double mean_factor = 1, double std_factor = 1)
{
    ...
    for (int i = 0; i < image.rows; ++i)
    {
        ...
        {
            double sum = image.at<Vec3b>(i, j)[0] + image.at<Vec3b>(i, j)[1] + image.at<Vec3b>(i, j)[2];
    ...
    for (int i = 0; i < patch_R.rows; ++i)
    {
        ...
        {
            double sum = image_patch.at<Vec3b>(i, j)[0] + image_patch.at<Vec3b>(i, j)[1] + image_patch.at<Vec3b>(i, j)[2];
    ...
    for (int i = 0; i < image.rows; ++i)
    {
        for (int j = 0; j < image.cols; ++j)
        {
            prob_R.at<double>(i, j) = exp(-pow(image_R.at<double>(i, j) - mean_factor * mean_patch_R, 2) / (2 * pow(std_factor * std_patch_R, 2))) / (std_factor * std_patch_R * sqrt(2 * M_PI));
            prob_G.at<double>(i, j) = exp(-pow(image_G.at<double>(i, j) - mean_factor * mean_patch_G, 2) / (2 * pow(std_factor * std_patch_G, 2))) / (std_factor * std_patch_G * sqrt(2 * M_PI));
        }
    }
