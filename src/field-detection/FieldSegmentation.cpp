...
void preprocessing(const Mat &src, Mat &dst, const double alpha_e)
{
    ...
    // Define the kernel size and shape (circular), which is dependent on the image size
    // double alpha_e = 0.5; // suggested 0.5
...
void green_chromaticity_analysis(const cv::Mat &src, cv::Mat &dst)
{
    ...
    dst.create(src.size(), CV_8UC1); // CV_8UC1 for single-channel (grayscale) image

    src.forEach<cv::Vec3b>([&dst](cv::Vec3b &pixel, const int *position) -> void
                           {
        ...
        float g = static_cast<float>(G) / (G + R + B);

        // Update the corresponding pixel in the destination matrix
        dst.at<uchar>(position[0], position[1]) = static_cast<uchar>(g * 255); });
}
...
{
    String filepath = "../model/trained_gmm_" + filename + "_NG_" + to_string(gmm->getClustersNumber()) + ".yml";
...
void train_gmm(Ptr<ml::EM> &gmm, const Mat &samples, const int N_G, Mat &log_likelihoods, Mat &labels, Mat &probs)
{
    gmm->setClustersNumber(N_G);
    gmm->setCovarianceMatrixType(ml::EM::COV_MAT_DIAGONAL);
    cout << "Training over " << samples.rows << " samples and " << to_string(N_G) << " Gaussians" << endl;
    ...
    cout << "Training time: " << training_time_seconds << " seconds. Saving to file" << endl;
}

Ptr<ml::EM> gmm_load_trained(const Mat &samples, const int N_G, Mat &log_likelihoods, Mat &labels, Mat &probs)
{
...
{
    return 1 / (sqrt(2.0 * CV_PI * variance)) * exp(-0.5 * pow(x - mean, 2) / variance);
}

Mat compute_pdf(const int N_G, const int num_points, const Mat &means, const vector<Mat> &covs, const Mat &weights, const Mat &g)
{
    ...
    for (int n_gaussian = 0; n_gaussian < N_G; ++n_gaussian)
    {
        ...
        for (int j = 0; j < num_points; j++)
        {
            ...
            pdf.at<double>(0, j) = gaussian_weight * compute_gaussian(g_value, gaussian_mean, gaussian_variance);
        }
...
Mat find_M_PF_hat(const double threshold, const Mat &chromaticity)
{
    ...
    for (int y = 0; y < chromaticity.rows; ++y)
    {
        for (int x = 0; x < chromaticity.cols; ++x)
        {
            ...
            double pixel_value = static_cast<double>(chromaticity.at<float>(y, x));
            // cout << pixel_value << endl;
            // Compare the pixel intensity with m0 and set to white if greater than T_G
...
int findFirstMaximumAfterThreshold(const Mat &pdf, const Mat &g, double threshold)
{
...
void visualize_gaussians(const vector<Mat> &covs, const Mat &means, const Mat &weights, int num_points, double T_G)
{
    ...
    for (int i = 0; i < covs.size(); ++i)
    {
        ...
        for (int j = 0; j < num_points; j++)
        {
            ...
            pdf.at<double>(0, j) = gaussian_weight * compute_gaussian(x, gaussian_mean, gaussian_variance);
        }

        // Find the maximum PDF value for this component
        double component_max_pdf = *max_element(pdf.begin<double>(), pdf.end<double>());
        if (component_max_pdf > max_pdf)
            max_pdf = component_max_pdf;
    }
    ...
    for (int i = 0; i < covs.size(); ++i)
    {
        ...
        for (int j = 0; j < num_points - 1; ++j)
        {
            ...
            int x2 = static_cast<int>((j + 1) * (plotWidth - 1) / (num_points - 1));
            int y1 = plotHeight - static_cast<int>(pdf.at<double>(0, j) * (plotHeight - 1));
            int y2 = plotHeight - static_cast<int>(pdf.at<double>(0, j + 1) * (plotHeight - 1));
    ...
    for (int i = 0; i <= 10; ++i)
    {
        ...
        line(plot, Point(x, plotHeight - 5), Point(x, plotHeight + 5), Scalar(255, 255, 255), 2);
        putText(plot, to_string(i * 0.1), Point(x - 10, plotHeight + 20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255, 255, 255), 1);
    }

    // Draw a vertical line at x = 1/3
    int x_vertical = static_cast<int>((1.0 / 3.0) * (plotWidth - 1));
    line(plot, Point(x_vertical, 0), Point(x_vertical, plotHeight), Scalar(0, 255, 0), 2);

    int Tg_vertical = static_cast<int>(T_G * (plotWidth - 1));
    line(plot, Point(Tg_vertical, 0), Point(Tg_vertical, plotHeight), Scalar(0, 0, 255), 2);
    // Show the plot
    imshow("PDF of Gaussian Components", plot);
    waitKey(0);
}

void show_pdf(const Mat &pdf, const int num_points, const Mat &g, const double threshold)
{
    ...
    for (int i = 0; i < num_points - 1; ++i)
    {
        ...
        int y1 = plotHeight - static_cast<int>(normalised_pdf.at<double>(0, i) * plotHeight);
        int y2 = plotHeight - static_cast<int>(normalised_pdf.at<double>(0, i + 1) * plotHeight);
    ...
    line(plot, Point(Tg_vertical, 0), Point(Tg_vertical, plotHeight), Scalar(0, 255, 255), 2);
    imshow("Probability Density Function of g", plot);
    waitKey(0);
}

void print_gaussians_data(const int N_G, const Mat &means, const vector<Mat> &covariances, const Mat &weights)
{
    std::cout.precision(3);
    for (int i = 0; i < N_G; i++)
        cout << "Gaussian #" << i + 1 << " has prior " << weights.at<double>(0, i) << ", \tvariance " << covariances[i].at<double>(0, 0) << ", \tmean " << means.at<double>(0, i) << endl;
}

Mat create_envelope(const vector<Mat> &covs, const Mat &means, const Mat &weights, int num_points, int N_G)
{
    // Set up the x-axis range
    ...
    for (int j = 0; j < num_points; j++)
    {
        // For each Gaussian
        for (int i = 0; i < N_G; i++)
            y_values.push_back(gaussians[i].at<double>(0, j)); // Create a vector of N_G values
        // Pick the max
        envelope.at<double>(0, j) = *max_element(y_values.begin(), y_values.end());
...
vector<LocalMinimum> findLocalMinima(const Mat &envelope, const Mat &g, const double threshold)
{
    ...
    for (int i = 1; i < num_elements - 1; ++i)
    {
        ...
        x = g.at<double>(0, i);
        if (current_element < prev_element && current_element < next_element && x > threshold)
        {
            LocalMinimum minimum;
            minimum.value = current_element;
            minimum.x = g.at<double>(0, i);
            cout << "Found local minimum at (x,y)=(" << minimum.x << " ," << minimum.value << ")" << endl;
...
vector<Vec3b> compute_mean_colors(vector<PixelInfo> pixels, vector<int> counts)
{
    vector<Vec3b> means;
    ...
    for (PixelInfo pixel : pixels)
    {
        ...
        if (group != 0)
            sums[group - 1] += Vec3f(pixel.pixel[0], pixel.pixel[1], pixel.pixel[2]);
    }

    for (int count = 0; count < counts.size(); count++)
    {
        Vec3f mean = sums[count] / static_cast<float>(counts[count]);
        Vec3b mean_color(static_cast<uchar>(mean_color[0]), static_cast<uchar>(mean_color[1]), static_cast<uchar>(mean_color[2]));
...
void plotEnvelope(const Mat &x_values, const Mat &envelope)
{
    ...
    for (int i = 0; i < x_values.cols - 1; ++i)
    {
        ...
        int x2 = static_cast<int>(x_values.at<double>(0, i + 1) * (plotWidth - 1));
        int y1 = plotHeight - static_cast<int>(envelope.at<double>(0, i) * plotHeight);
        int y2 = plotHeight - static_cast<int>(envelope.at<double>(0, i + 1) * plotHeight);
...
Mat chromatic_distorsion_matrix(const Mat &I_gca, const Mat &I_open, const double T_G, const vector<LocalMinimum> &minima)
{
    ...
    // Assign each pixel to a cluster based on the information in the pixels vector
    for (int y = 0; y < I_open.rows; ++y)
    {
        for (int x = 0; x < I_open.cols; ++x)
        {
            // Get the cluster assignment from the corresponding pixel in the pixels vector
            int cluster = pixels[y * I_open.cols + x].group;

            // Assign the cluster value to the corresponding location in the group_matrix
            group_matrix.at<uchar>(y, x) = static_cast<uchar>(cluster);
        }
    }

    I_open.forEach<Vec3b>([&cd_matrix, &group_matrix, &avgs](Vec3b &pixel, const int *position) -> void
                          {
            float cd_value = 1.0;
